<?php 
  ...
  if($status=="activated")
  {
    ...
    $conn->query("update sub_event set status='deactivated' where subevent_id='$sub_event_id'");?>
                ...
                alert('Category: <?php echo $se_name; ?> deactivated successfully!');						
                </script>
...
$cont_query = $conn->query("SELECT * FROM contestants WHERE subevent_id='$sub_event_id'") or die(mysqli_error());
if($cont_query->rowCount()>0)
{
    ?>
                <script>
                alert('Category: <?php echo $se_name; ?> activated successfully!');	
